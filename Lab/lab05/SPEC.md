> **Source**: Berkeley CS61C Fall 2020 (fa20) — official lab spec
> **Archived at**: https://web.archive.org/web/20220121163317/https://inst.eecs.berkeley.edu/~cs61c/fa20/labs/lab05/
> **Note**: 以下为官方要求原文（经 Wayback Machine 存档），仅供个人学习参考。
...
![](./lab5img/zoom_button.png)

1.  ![](/~cs61c/fa20/labs/lab05/lab5img/and.gif) Start by clicking the `AND` gate button. This will cause the shadow of an `AND` gate to follow your cursor around. Click once within the main schematic window to place an `AND` gate.

2.  ![](/~cs61c/fa20/labs/lab05/lab5img/input.gif) Click the `Input Pin` button. Now, place two input pins somewhere to the left of your `AND` gate.

3.  ![](/~cs61c/fa20/labs/lab05/lab5img/output.gif) Click the `Output Pin` button. Then place an output pin somewhere to the right of your `AND` gate. Your schematic should look something like this at this point:

    ![](/~cs61c/fa20/labs/lab05/lab5img/schematic1.gif)

4.  ![](/~cs61c/fa20/labs/lab05/lab5img/selection.gif) Click the `Select tool` button. Click and drag to connect the input pins to the left side of the `AND` gate. This will take several steps, as you can only draw vertical and horizontal wires. Just draw a wire horizontally, release the mouse button, then click and drag starting from the end of the wire to continue vertically. You can attach the wire to any pin on the `AND` gate on the left side. Repeat the same procedure to connect the output of the `AND` Gate (right side) to the output pin. After completing these steps your schematic should look roughly like this:

    ![](/~cs61c/fa20/labs/lab05/lab5img/schematic2.gif)

5.  ![](/~cs61c/fa20/labs/lab05/lab5img/poke.gif) Finally, click the `Poke` tool and try clicking on the input pins in your schematic. Observe what happens. Does this match with what you think an `AND` Gate should do? Note that poking the wires themselves tells you the current value on that wire; this will be very useful later when you build more complex circuits.
...
5.  Select the register from the `Memory` folder and place one register into your subcircuit. Below is an image diagraming the parts of a register.
    ...
    ![](/~cs61c/fa20/labs/lab05/lab5img/register.gif)
...
9.  Connect the two output pins to your circuit so that you may monitor what comes out of the adder and the register. The output of the adder should be connected to `ADD_OUT` and the output of the register to `REG_OUT`. Thus, by the end, your circuit should look like as follows:
    ...
    ![](/~cs61c/fa20/labs/lab05/lab5img/AddMachine.png)
...
1.  Note that the FSM is implemented by the following diagram (the four state names `00`, `01`, `10`, `11` are just names written in binary - they have no direct relationship with the actual zeros and ones of the FSM input/output). Take some time to understand how this diagram implements the FSM:
    ...
    ![](/~cs61c/fa20/labs/lab05/lab5img/FSM.png)
...
![](./lab5img/left_split_menu.png)

While the right splitter’s menu looks like this:

![](./lab5img/right_split_menu.png)
...
![](/~cs61c/fa20/labs/lab05/lab5img/orange_error.png)
...
![](/~cs61c/fa20/labs/lab05/lab5img/extend1.png)

Whereas the following is much simpler, easier to read, and less error-prone:

![](/~cs61c/fa20/labs/lab05/lab5img/extend2.png)
...
2.  Change the `Bit Width In` property (bus width) to `8`, and `Fan Out` property (# of branches) to `3`. Your splitter should now look as follows:
    ...
    ![](/~cs61c/fa20/labs/lab05/lab5img/splitter.gif)
...
- [CS 61C](/~cs61c/fa20)
- [Calendar](/~cs61c/fa20/calendar)
- [Staff](/~cs61c/fa20/staff)
- [Policies](/~cs61c/fa20/policies)
- [Piazza](https://piazza.com/)
- [OH Queue](https://oh.cs61c.org)
- [Venus](https://venus.cs61c.org/)
- [Resources](/~cs61c/sp21/resources/)
- [Semesters](https://inst.eecs.berkeley.edu/~cs61c/archives.html)
- [Back to top](#)
